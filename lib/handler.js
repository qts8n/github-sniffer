...
export const handleUrl = (msg, responseHandler) => {
    ...
    if (!~Object.keys(URL_HANDLERS).indexOf(cmd)) return;
    const url = words[1];
...
export const handleMessage = msg => {
    ...
    switch (command) {
        ...
        case '!check': checkForUpdates(msg.client, true); throw new Interrupt();
        ...
        case '!help':
            msg.reply(`
                ...
                \`!ping\` checks bot online status
                \`!list\` gets current watchlist
                \`!add <URL>\` adds github user to the watchlist
                \`!remove <URL>\` removes github user from the watchlist
                \`!check\` does checkout for updates out of schedule
            `);
            break;
        default: break;
    }
...
export const checkForUpdates = (client, isTalkative = false) => manageWatchList(console.error)(records => {
    let promises = [];
    ...
    Promise.all(promises).then(users => {
        ...
        if (results.length === 0) return isTalkative && channel.send('@everyone, No updates since last check');
